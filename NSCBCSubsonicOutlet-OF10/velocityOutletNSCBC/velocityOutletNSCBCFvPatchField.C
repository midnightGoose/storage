...
//
// The velocity LODI relation (Eq. 26) governs the normal component:
//
//     du/dt + (L5 - L1)/(2*rho*c) = 0
//
// L5 is the outgoing acoustic wave (Eq. 23); L1 is the incoming acoustic
// wave estimated via pressure relaxation toward pInf (Section 3.2):
//
//     L1 = K*(p - pInf),   K = etaAc*(1 - M^2)*c/lInf
//
// The gradient relation (Eq. 35) provides the acoustic contribution to the
// normal velocity gradient at the face:
//
//     du/dn = -(1/(rho*c)) * dp/dn
//
// Transverse velocity components advect at speed u and require no separate
// treatment at a subsonic outlet (LODI Eqs. 27-28 give dv/dt = dw/dt = 0
// when L3 = L4 = 0, consistent with zero-gradient for those components).

...
// NSCBC subsonic outlet — pressure field (v2412 port).
//
// Incoming acoustic wave relaxation toward pInf [P&L Eq. 40 / BPT Eq. 26]:
//
//     L1 = K*(p - pInf),   K = etaAc*(1 - M^2)*c/lInf
//
// Outgoing acoustic wave from one-sided interior gradients [P&L Eq. 23]:
//
//     L5 = (u+c)*(dp/dn + rho*c*du/dn)
//
// Pressure LODI [P&L Eq. 25]:
//
//     dp/dt + (L1 + L5)/2 = 0
...
void Foam::pressureOutletNSCBCFvPatchField<Type>::updateCoeffs()
{
    ...
    if (lInf_ > 0)
    {
        ...
        else
        {
            FatalErrorInFunction
                ...
                << "' in pressureOutletNSCBC on patch " << this->patch().name()
                << exit(FatalError);
        }
    }
    else
    {
        FatalErrorInFunction
            << "lInf must be > 0 on patch " << this->patch().name()
            << " of field " << this->internalField().name()
            << " in file " << this->internalField().objectPath()
            << exit(FatalError);
    }
